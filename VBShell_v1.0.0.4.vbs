...
Function Import(ByVal FP)
    ...
    InFuc = 0: Count = -1: Code = Split(FSO.OpenTextFile(FP).ReadAll(), vbCrLf)
    For Each Line In Code
        If LCase(Left(LTrim(Line), 9)) = "function " Or LCase(Left(LTrim(Line), 4)) = "sub " Or LCase(Left(LTrim(Line), 6)) = "class " Or LCase(Left(LTrim(Line), 12)) = "private sub " Or LCase(Left(LTrim(Line), 17)) = "private function " Or LCase(Left(LTrim(Line), 11)) = "public sub " Or LCase(Left(LTrim(Line), 16)) = "public function " Or LCase(Left(LTrim(Line), 16)) = "public property " Or LCase(Left(LTrim(Line), 17)) = "private property " Then InFuc = InFuc + 1
        If InFuc > 0 Then 
            Count = Count + 1
            If Count = 0 Then ReDim Funcs(Count) Else ReDim Preserve Funcs(Count)
            Funcs(Count) = Line
        End If
        If LCase(Left(LTrim(Line), 12)) = "end function" Or LCase(Left(LTrim(Line), 7)) = "end sub" Or LCase(Left(LTrim(Line), 9)) = "end class" Or LCase(Left(LTrim(Line), 12)) = "end property" Then InFuc = InFuc - 1
    Next
